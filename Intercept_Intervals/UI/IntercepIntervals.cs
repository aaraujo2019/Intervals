...
{
    public partial class IntercepIntervals : Form
    {
        DataTable dtCollars;
        private string excel_template_name = ConfigurationManager.AppSettings["excel_template_name"].ToString();
        
        #region constructor
        public IntercepIntervals()
        {
            InitializeComponent();
            FillcbxHole();
            fillComboBoxVhmod();
        }
        public IntercepIntervals(string User)
        {
            InitializeComponent();
            FillcbxHole();
            fillComboBoxVhmod();
        ...
        private void IntercepIntervals_Load(object sender, EventArgs e)
        {
            Clear();
        ...
        private DataTable getEmptyRow()
        {
            ...
            String[] headers = "Selection,Vn_mod,from,to,LENGTH,sampno,lithology,au_ppm,ag_ppm,dhid,jobno,lab,aufaa_ppm,aufagr_gpt,agfa_ppm".Split(',');
        ...
        private void cmbHoleID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbx.SelectedIndex != 0)
            {
                ...
                if (dtCollars.Rows.Count > 0)
                {
                    ...
                    if (dtCollars.Select("IsSaved = 1").Count() > 0)
                    {
                        loadgridCalculated();
        ...
        private void dtgDetailHoleID_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
        ...
        private void exportByTarget(string target, string sources)
        {
            ...
            if (dtCollarsByTarget.Rows.Count > 0)
                exportDataTableBySourceOrTarget(dtCollarsByTarget, "Exploration Target: " + target + " and sources : " + sources);
            else
            {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        oSheet.Cells[initialRowCellidx, 12] = (dtgValueCalculate.Rows[idx].Cells[2] != null) ? dtgValueCalculate.Rows[idx].Cells[2].Value.ToString() : "";
                        oSheet.Cells[initialRowCellidx, 13] = (dtgValueCalculate.Rows[idx].Cells[3] != null) ? dtgValueCalculate.Rows[idx].Cells[3].Value.ToString() : "";
                        oSheet.Cells[initialRowCellidx, 14] = (dtgValueCalculate.Rows[idx].Cells[4] != null) ? dtgValueCalculate.Rows[idx].Cells[4].Value.ToString() : "";
                        oSheet.Cells[initialRowCellidx, 15] = (dtgValueCalculate.Rows[idx].Cells[5] != null) ? dtgValueCalculate.Rows[idx].Cells[5].Value.ToString() : "";
                        oSheet.Cells[initialRowCellidx, 16] = (dtgValueCalculate.Rows[idx].Cells[6] != null) ? dtgValueCalculate.Rows[idx].Cells[6].Value.ToString() : "";
